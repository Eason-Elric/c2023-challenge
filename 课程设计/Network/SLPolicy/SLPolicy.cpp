...
inline SLPolicy::SLPolicy(int GPU_id) {
...
inline void SLPolicy::Initialize(int InChannels, int OutChannels, const std::string& PretrainedPath) {
    Module = SLPolicyModule(InChannels, OutChannels);
    if (!PretrainedPath.empty()) {
        auto NetPretrained = SLPolicyModule(InChannels, OutChannels);
...
inline void SLPolicy::LoadWeights(const std::string &Path) {
    torch::load(Module, Path);
    Module -> eval();
}

inline void SLPolicy::Train(int Samples, int Epochs, int BatchSize, float LearningRate) {
...
inline map<Grid, float> SLPolicy::Predict(BoardState boardState) {
